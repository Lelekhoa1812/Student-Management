...
import { FileText, Plus, Trash2, Save, Eye, AlertCircle, CheckCircle } from "lucide-react"
...
export default function CreateExamPage() {
  ...
  const addMappingColumn = (questionIndex: number, columnType: 'left' | 'right') => {
    ...
    // Debug log
    console.log(`Added ${columnType} column to question ${questionIndex}:`, newColumn)
  ...
  const updateMappingColumn = (questionIndex: number, columnIndex: number, field: keyof MappingColumn, value: string) => {
    ...
    const question = updatedQuestions[questionIndex]
    if (question.mappingColumns) {
      console.log(`Updating mapping column ${columnIndex} in question ${questionIndex}:`, { field, value })
      console.log('Before update:', question.mappingColumns[columnIndex])
      
      question.mappingColumns[columnIndex] = { ...question.mappingColumns[columnIndex], [field]: value }
      setQuestions(updatedQuestions)
      
      console.log('After update:', question.mappingColumns[columnIndex])
    }
  }
  ...
  const removeMappingColumn = (questionIndex: number, columnIndex: number) => {
    ...
    if (question.mappingColumns && question.mappingColumns.length > 1) {
      console.log(`Removing mapping column ${columnIndex} from question ${questionIndex}`)
      console.log('Before removal:', question.mappingColumns)
      
      const removedColumn = question.mappingColumns[columnIndex]
      question.mappingColumns.splice(columnIndex, 1)
      setQuestions(updatedQuestions)
      
      console.log('Removed column:', removedColumn)
      console.log('After removal:', question.mappingColumns)
    }
  }
  ...
  const saveTest = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(error.error || "Có lỗi xảy ra khi tạo đề thi")
      }
    } catch (error) {
            ...
            {/* Questions Section */}
                    ...
                    {question.questionType === 'mapping' && (
                      <div className="space-y-3">
                        ...
                        {/* <div className="text-xs text-gray-500 bg-gray-100 p-2 rounded">
                          <p>Debug: Tổng số cột: {question.mappingColumns?.length || 0}</p>
                          <p>Trái: {question.mappingColumns?.filter(col => col.columnType === 'left').length || 0}</p>
                          <p>Phải: {question.mappingColumns?.filter(col => col.columnType === 'right').length || 0}</p>
                          <p>Chi tiết: {JSON.stringify(question.mappingColumns?.map((col, idx) => ({ idx, type: col.columnType, text: col.itemText })))}</p>
                        </div> */}
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            <Label className="text-sm font-medium">Cột trái</Label>
                            {question.mappingColumns?.filter(col => col.columnType === 'left').map((col, filteredIndex) => {
                              ...
                              return (
                                <div key={`left-${actualIndex}`} className="flex items-center space-x-2">
                                  ...
                                  <Input
                                    value={col.itemText}
                                    onChange={(e) => updateMappingColumn(questionIndex, actualIndex, 'itemText', e.target.value)}
                                  ...
                                  {question.mappingColumns && question.mappingColumns.length > 2 && (
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      onClick={() => removeMappingColumn(questionIndex, actualIndex)}
                                      className="text-red-600 hover:text-red-700"
                                    >
                                      <Trash2 className="w-4 h-4" />
                                    </Button>
                          ...
                          <div className="space-y-2">
                            ...
                            {question.mappingColumns?.filter(col => col.columnType === 'right').map((col, filteredIndex) => {
                              // Find the actual index in the original array
                              const actualIndex = question.mappingColumns!.findIndex(c => c === col)
                              return (
              ...
              {questions.length === 0 && (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>Chưa có câu hỏi nào. Hãy thêm câu hỏi để bắt đầu tạo đề thi.</p>
                </div>
              )}
            </div>

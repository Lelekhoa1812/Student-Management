...
export default function ManagerInfoPage() {
  ...
  const [editedData, setEditedData] = useState<Partial<ManagerData>>({})
  ...
  const fetchManagerData = async () => {
    try {
      const response = await fetch(`/api/manager?email=${session?.user?.email}`)
      if (response.ok) {
      ...
      setError("Không thể tải thông tin manager")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`/api/manager/${managerData.id}`, {
        method: "PUT",
        headers: {
        ...
        <div className="container mx-auto px-4 max-w-2xl py-8">
          <Card>
            <CardContent className="text-center py-8">
              <p className="text-gray-600">Không tìm thấy thông tin manager</p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="p-3 bg-gray-50 rounded-md">
                  {managerData.role === "manager" ? "Quản lý" : managerData.role}
                </div>
              </div>
            ...
            <div className="mt-6 space-y-2">
              ...
              <div className="p-3 bg-gray-50 rounded-md">
                {new Date(managerData.createdAt).toLocaleDateString('vi-VN')}
              </div>
            </div>
